...
(() => {
  proxy({
    onResponseReady: (xhr) => {
      ...
      if (lang) {
        const key = tlang ?? lang;
        const isAutoGenerated = url.searchParams.get("kind") === "asr";
        if (!isAutoGenerated || !subtitleMap.has(key)) {
          subtitleMap.set(key, xhr);
        }
      } else {
  ...
  function addSubtitleExtensionElements() {
    ...
    const joinTimedText = (segments) => segments.flatMap((e) => e.segs.map((s) => s.utf8.replace(/\n/g, ""))).join(" ");
